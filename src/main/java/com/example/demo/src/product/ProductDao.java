...
public class ProductDao {
    ...
    public List<GetProductList> getProductsByUserNo(int userNo, int productNo){
        String getProductsQuery = "select " +
                "Product.productNo," +
                "User.userNo,User.userNickName," +
                "case " +
                ...
                "join Room " +
                "join UserMainRegion on User.userRegionNo = UserMainRegion.userMainRegionNo " +
                "where productTitle != 'null' and User.userNo = ? and Product.productNo != ? " +
                "group by Product.productNo " +
                "order by Product.createdAt desc";
        int getProductByUserNoParams = userNo;
        ...
        return this.jdbcTemplate.query(getProductsQuery,
                (rs,rowNum) -> new GetProductList(
                        ...
                        rs.getInt("productViewNum"),
                        rs.getInt("chatNum")),
                getProductByUserNoParams, getProductByProductNoParams
        );
    }
...
/*    public List<GetProductList> getProductNosByProductNo(int productNo){
    ...
    public GetProductDetail getProductDetail(int productNo, int productUserNo){
        ...
        return this.jdbcTemplate.queryForObject(getProductDetail,
                (rs, rowNum) -> new GetProductDetail(
                        ...
                        rs.getString("productStatus"),
                        rs.getString("productPrice"),
                        rs.getString("productPriceStatus"), getProductsByUserNo(productUserNo,productNo))
                        ,
                getProductDetailParams);
    }
    public GetProductUser getUserNo(int productNo){
        String getIdQuery = "select productNo, userNo from Product where productNo = ? ";
        int getNoParams = productNo;
        return this.jdbcTemplate.queryForObject(getIdQuery,
                (rs,rowNum)-> new GetProductUser(
                        rs.getInt("productNo"),
                        rs.getInt("userNo")
                ),
                getNoParams);
